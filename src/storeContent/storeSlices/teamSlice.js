...
export const saveTeam = createAsyncThunk("teams/save", async (team, { rejectWithValue }) => {
    try {
...
export const checkTeams = createAsyncThunk("teams/check", async (mapping, { rejectWithValue }) => {
    try {
        console.log("SENDING : ", Object.fromEntries(mapping));
        const response = await axios.patch(`${baseUrl}/api/data/teams`, Object.fromEntries(mapping));
        return response.data;
    }
    catch (error) {
        // return rejectWithValue(error.message);
...
export const deleteTeam = createAsyncThunk("teams/delete", async (teamId, { rejectWithValue }) => {
    try {
        const response = await axios.delete(`${baseUrl}/api/data/teams/${teamId}`);
        return response.data;
    }
    catch (error) {
...
export const TeamSlice = createSlice({
    ...
    reducers: {
        ...
        checkTeam: (state, action) => {
            state.teams = state.teams.map((team) => team.id !== action.payload.id
                ? team
                : { ...team, isChecked: action.payload.isChecked || false });
        },
        checkAllTeams: (state, action) => {
